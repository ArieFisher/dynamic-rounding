...
This repository is public. Before staging a new file, answer: does it serve someone using or building DynamicRounding? If the answer is no, it does not go in — and a report about my local toolchain, an agent scan, a session journal, or a scratch note is always no. 
...
- **Imagery:** no personification. The full rule, with its test and examples, is the Imagery bullet in `AGENTS.md` under Writing style; that file holds the only copy.
