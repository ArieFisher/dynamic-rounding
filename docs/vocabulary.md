...
| cell cap | The most cells one pass of the re-apply observer reads: a native table's cells, a grid's visible cells. The detection settings hold it as `reapplyCellCap`, 3,000 today, which keeps a pass under 100 ms. Above it the pass writes nothing, the observer stops for that table, and one warning row records the stop. The first simplification has no cell cap. |
