...
| stacked cell | A table cell whose text pieces each hold one whole number or unit number, or nothing but whitespace, a currency sign, a percent sign, or a listed currency code: "125" above "126". Each number rounds in its own piece. A piece that reads as a date or a time makes the cell not stacked. Two pieces that the rendered text runs together, with no space or line break between them, hold one number split across them. |
