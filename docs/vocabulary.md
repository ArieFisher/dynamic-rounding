...
| magnitude freeze | On a virtualized grid, max magnitude is fixed based on the content in the table when simplification is first applied. Scrolling new rows into view does not shift the max magnitude, even if the new data contains bigger OoM numbers. <br><br>A native table has no magnitude freeze: every pass computes its max magnitude again, so a larger value the page writes or adds changes the rounding of the whole table. A grid holds only its visible rows, so a changed value and a row scrolled into view read the same. |
...
| currency list | The one list of the currencies the extension reads: each row gives a currency's written signs and its ISO code. Every currency rule reads it — reading a cell as a number, the currency exclusion, the sign-only piece test, the data test's numeric probe, and putting the sign back after rounding. A sign written in letters counts only as its own token, so the rand's "R" counts in "R45" and not in "Revenue". |
| format mark | A character that sits beside a number without belonging to it: a currency sign, a percent sign, whitespace. The one list lives beside the currency list, and every rule that steps past a mark to reach a number reads it — stripping the marks before a text reads as a number, and testing what stands between a bracket and the number. A bracketed number's brackets are its minus sign, not format marks. |
| bracketed number | A number whose minus sign is written as the brackets around it, the accounting form: "(1,234)" is -1,234. The brackets hold nothing but the number and its own format marks, so "($1,234)" and "(12.3%)" are bracketed numbers and "(see note 4)" is not. Its digits alone round and the brackets stay where the page put them, so a bracket in its own text piece never holds the cell back. A bracket pair followed by a digit is a telephone area code, not a minus sign. It rounds on every table kind, whatever the sidebar's "words" setting holds. |
| unit number | A cell whose whole text is one number with a magnitude suffix after it, a listed currency code before or after it, or both: "4.91tn", "CAD$45.67", "1,234 USD". Its digits round and the suffix and code stay. It rounds on every table kind, whatever the sidebar's "words" setting holds. The codes come from the currency list; the suffixes are listed once, in the number parser. |
| stacked cell | A table cell whose text pieces each hold one whole number or unit number, or nothing but whitespace, a currency sign, a percent sign, or a listed currency code: "125" above "126". Each number rounds in its own piece. A piece that reads as a date or a time makes the cell not stacked. On a native table, two pieces that the rendered text runs together, with no space or line break between them, hold one number split across them. |
| split number | A number whose characters sit across two text pieces: "4." in one and "91" in the next. It stays unchanged. |
| allow list | A list of the characters or words that may touch a number in an extracted cell, so an identifier such as "DT1234" stays unchanged. Not built yet; until it exists, an extracted cell rounds every number it finds outside an identifier shape, on every table kind. |
| at-name | A name written after "@", such as a social media name ("@2020vision") or the name part of an email address. A number right after "@" belongs to the at-name and never rounds, in any cell. |
| identifier shape | A text pattern that names a thing instead of counting it: a phone number ("416-555-1234", "(416) 555-1234"), an IP address ("192.168.0.1"), a web or email address, an ISBN ("ISBN 978-0-306-40615-7"), a postal code ("M5V 2T6", "90210-1234"), or digit groups split by whitespace outside thousands grouping ("416 555 1234"). A cell matching one stays as written on every table kind, whatever the sidebar's "words" setting holds. Inside an extracted cell, a shape keeps its digits and the cell's other numbers round: "Call 416-555-1234 about 1,613,245 units" keeps the phone number. Two shapes count only as a whole cell: digit groups split by spaces alone, and an ISBN without the word "ISBN". The shapes are listed once, in the number library. |
...
| re-apply observer | The watcher that detects a page change inside a simplified table (a virtualized grid redrawing its cells, a page rewriting a value or adding a row) and applies the rounding again, on native tables and grids alike. It runs no pass for the extension's own writes, collapses a burst of page edits into one pass, and stops for a table above the cell cap. |
| cell cap | The most cells one pass of the re-apply observer reads: a native table's cells, a grid's visible cells. The detection settings hold it as `reapplyCellCap`, 10,000 today, which keeps a pass under 100 ms. Above it the pass writes nothing, the observer stops for that table, and one debug row records the stop, with no toast. The first simplification has no cell cap. |
...
| patch writer | The write step that applies patches, each inside one text piece. It applies a cell's patches to the cell's originals and writes each text piece whose text differs from the result. A patch whose characters are not at its position in the originals does not land. |
| placement step | The step between classification and the patch writer: it checks a cell's classified result against the cell's text pieces. A result whose changed characters sit in one text piece stands. Otherwise the cell takes the stacked cell test. |
| originals | The cell values stored before simplification so the table can be restored. A cell's originals hold each text piece's text. |
| written text | The text the extension last wrote into a text piece, stored beside the piece's original. A pass compares each piece's text with its original and its written text to tell a cell the extension still holds from a rewritten cell. |
| rewritten cell | A simplified cell whose text the page changed: a text piece shows neither its original nor its written text, or the cell holds a different count of text pieces. A pass puts the originals back into the pieces that still show written text, drops the cell's originals, and simplifies the cell again from the page's text. When the count of text pieces changed, a piece matches its stored piece by written text, in page order, instead of by position. |
