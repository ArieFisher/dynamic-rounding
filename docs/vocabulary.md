...
| qualifying element | An element carrying a grid or table role that passes the two guards the ARIA pass applies: it is not a native table, and it holds no native table that the accessibility-artifact check keeps. The data test has not run on it yet. |
| chain root | The outermost qualifying element of a nest: a qualifying element with no qualifying ancestor. |
| containment chain | The qualifying elements nested under a chain root, the root included, that pass the data test, grouped by nesting depth. An element's depth is the count of qualifying ancestors between it and the chain root, and the root sits at depth 0. |
