...
| allow list | A list of the characters or words that may touch a number in an extracted cell, so an identifier such as "DT1234" stays unchanged. Not built yet; until it exists, an extracted cell rounds every number it finds, identifier or quantity alike, on every table kind. |
