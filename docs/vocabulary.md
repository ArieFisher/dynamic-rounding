...
| shape fingerprint | The column count and the header row's cell texts, recorded for a table when it enters the registry. Every action on a registered table compares the table's current shape against it first. A mismatch means the page replaced the table's content with a different shape, so the entry is discarded and the table registers fresh. The column count is the widest row's cell count, and the header row is the first row the adapter returns. The row count stays out, because a virtualized grid changes its drawn row count on every scroll. |
