...
| note | A reader hint in the capture file: one sentence in italics, one size below the table text, opening with "Note: ", stating a limit of what the file shows (the hover reveal, unrecorded cell spans, missing service worker rows, the seed's escaping). One helper renders every note, so the prefix and the style live in one place. An absence is never a note: it renders in the absence box. Lens preview lines are content, never notes. |
