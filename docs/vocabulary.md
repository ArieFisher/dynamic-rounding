...
| tuning block | The constants.js object holding every detection tuning value and the two lookup lists detection reads. Detection, the pillbox view, and the controller read it as a bare global, with no local fallback copy of any value. |
