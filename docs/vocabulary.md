...
| pending table | A chain root whose containment chain is empty: no qualifying element of its nest passes the data test, so the nest registers nothing yet. One subtree observer watches the chain root, and every change to the subtree runs the nomination step from the root again. A re-test that registers an element ends the pending table, and so does a re-test that finds the nest already registered or its configured depth crowded. A re-test that finds the chain still empty counts against the re-test cap, and reaching the cap drops the observer. <br><br>This is what registers a grid that arrives before its rows. |
