...
| format mark | A character that sits beside a number without belonging to it: a currency sign, a percent sign, whitespace, and the brackets of a bracketed number. The one list lives beside the currency list, and every rule that steps past a mark to reach a number reads it — reading a cell as a number, and the bracket test. |
