...
**The content script holds the model.** Its lifetime matches the page, and it is where the tables are. The application model holds the active table, the settings record, and the registry. Every read goes through a getter. Every write goes through a setter, and each of the two scalar setters publishes the field's whole new value on the event bus.
