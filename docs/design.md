...
Redrawing runs two ways. Direct calls from the controller redraw the pillbox. The sidebar records the tab it was opened for, pulls current values from that tab, and after that redraws on the state-change topics it subscribes to. It serves that one tab: a report reaching it from any other tab is dropped, because a content script reports by broadcast and a broadcast names no tab. While another tab is in front, the sidebar's controls describe a page the user is not looking at, so they give way to a message and come back from a fresh read on the return. The service worker usually closes the sidebar on a tab switch before that happens. The message is what shows when it does not: an idle restart empties the worker's tab number, and a sidebar opened from Chrome's own side-panel control never sets it. Which of the two happens is not predictable from the user's side, and the second part of the 2026-09-14 spec is where that gets settled. The state-change topic for the active table has no subscriber at all; the third part of the 2026-09-14 spec gives it one.
