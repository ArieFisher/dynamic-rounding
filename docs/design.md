...
4. **Robust:** Handles empty strings, zeros, nulls, and dates without crashing.
   ...
   - *Chrome extension:* A cell the classification ladder refuses is left untouched. Date cells are actively simplified (to the year by default); time cells are opt-in. A unit number — one number with a magnitude suffix (k, m, b, t, bn, tn) or a listed currency code such as CAD — rounds its digits and keeps the rest. Google Sheets and Python do not read suffixes or codes.
