...
4. **Robust:** Handles empty strings, zeros, nulls, and dates without crashing.
   ...
   - *Chrome extension:* A cell the classification ladder refuses is left untouched. Date cells are actively simplified (to the year by default); time cells are opt-in. A unit number — one number with a magnitude suffix (k, m, b, t, bn, tn) or a listed currency code such as CAD — rounds its digits and keeps the rest. A bracketed number — the accounting negative, "(1,234)" — rounds its digits too, and the brackets stay where the page put them. A cell whose whole text is an identifier shape — a phone number, an IP address, a web or email address, an ISBN, a postal code, or digit groups split by spaces outside thousands grouping — stays as written. Inside a cell with words, the same shapes keep their digits and the other numbers round; spaced digit groups and an ISBN without the word "ISBN" count only as a whole cell. Google Sheets and Python do not read suffixes or codes, and they read "416 555 1234" as one number (#464).
...
| Package | Job |
|---------|-----|
| `constants.js` | The settings contract and the detection settings, loaded by the content script and the sidebar: each option's name and its default, and every detection setting with its two lookup lists |
...
**The cell values yield the derived data**, with one deliberate exception. The date format hint, the magnitudes, and the dataset's maximum all come from the values. On a virtualized grid only the visible rows exist, so the maximum magnitude freezes at the first simplification and the registry holds it; recomputing would shift the basis on every scroll. A grid holds only its visible rows, so a changed value and a row scrolled into view read the same, and a larger value takes the top band's rounding under the frozen basis. A native table holds every row, so each pass computes its maximum again.
...
- **An apply restores the table before simplifying it again**, so a settings change produces a fresh pass. The re-apply observer takes a different route: it sorts each cell by its record and writes each text piece whose text differs from its target, reading the originals through the adapter's port.
- **One restore serves both table kinds.** It puts each original back into the text piece that still shows its written text. A patch changes only text, never tags, so the cell's markup comes back as it stood; no copy of the markup is stored. A piece the page rewrote keeps the page's text, so a restore that arrives before a pass never writes a number the page no longer shows. When the page redrew a cell with a different count of text pieces, the pieces no longer line up by position, so each piece that shows a stored piece's written text takes that piece's original, matched in page order, one stored piece to one live piece. A redraw that adds a tag around a rounded number therefore still restores the number.
- **One pass simplifies every table.** The first simplification and every re-apply pass, on either table kind, run the same pass inside the controller, in phases: release rewritten cells, classify every cell, resolve ambiguous dates per column, find the max magnitude, then build and write each cell's patches in one loop. Every difference between the table kinds is a setting of that pass: where the max magnitude comes from (the cells, or a grid's magnitude freeze), which debug rows a split value writes, whether a written cell carries hover text, and which element the re-apply observer watches (a native table itself, a grid's scroll container). Both kinds store the same record per cell: the text it classified, each text piece's original and written text, and its superscript ranges and link-filter result. A rule added to the pass therefore reaches both kinds at once, and the first simplification and the re-apply cannot diverge.
