...
- **One pass simplifies every table.** The first simplification of a native table, the first simplification of a grid, and the grid's scroll re-apply run the same pass inside the controller, in phases: classify every cell, resolve ambiguous dates per column, find the max magnitude, then build and write each cell's patches in one loop. Every difference between the table kinds is a setting of that pass: where the max magnitude comes from (the cells, or a grid's magnitude freeze), whether the placement step runs the stacked cell test, how the superscript ranges are measured, which debug rows a split value writes, and how a written cell's originals, hover text, and marker class are stored. A rule added to the pass therefore reaches both kinds at once, and the first simplification and the re-apply cannot diverge.
