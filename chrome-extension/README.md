...
- A **stacked cell** rounds number by number, on both table kinds: "125" above "126" becomes "150" above "150", and a "$" in its own piece beside "337.91" stays while the number becomes "350". A stacked cell rounds with the sidebar's "words" setting off too. Two numbers in one piece, even with a space between them, make the cell not stacked, so it stays unchanged. So does a piece that reads as a date or a time: "2024" above "2025" stays, as a lone "2024" does.
...
A digit beside a digit across two pieces holds either two numbers on separate lines or one number that inline styling splits ("6,7" plain, "18,245" in bold). The cell's rendered text sorts the two shapes: it holds a line break or a space between pieces on separate lines, and nothing between pieces that inline styling splits. Both table kinds read it for this check, so both round the two numbers on separate lines and leave the styled number unchanged as a split number. A grid otherwise classifies its flat text, so the check reads the rendered text only for a cell that reaches it. A native cell whose rendered text differs from its pieces in more than whitespace, such as a cell with a hidden sort key, takes no stacked cell test.
