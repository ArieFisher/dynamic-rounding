...
We are pretty good at doing that - the brain continually searches for patterns and unconsciously interprets them - we just need a little help sometimes.
...
The extension rounds numbers in place on any web page. Every data table it finds gets a small on/off control (the pillbox); a right-click menu and a side panel (the sidebar) hold the full controls, with a live preview of the rounding. It handles native HTML tables and virtualized data grids (Databricks SQL, AG Grid), and simplifies dates to the year by default.
...
* `=ROUND_DYNAMIC(A1:A10)` → rounds the whole dataset with set-aware precision
