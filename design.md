...
Instead of having to call 3 functions, the user can call this one function in 3 different modes depending on their choice of parameters:

- **Single mode:** first parameter is a value, second is a number or empty
- **Dataset mode:** first parameter is an array
- **Dataset-aware single mode:** first parameter is a value, second is an array
...
| Mode | Description |
|------|-------------|
| Single | Rounds one value based on its own magnitude. |
| Dataset | Rounds a range. Applies different offsets to top magnitude(s) vs others. |
| Dataset-aware single | Same logic as dataset mode, but for one value within context of a range. Safe to sort. |
...
| Offset | How many orders of magnitude to shift from the current OoM when rounding. Negative = finer, positive = coarser. |
| Magnitude | Shorthand for order of magnitude. Calculated as `floor(log10(abs(value)))`. |

### Offsets

The "declarative" nature of this approach to rounding works based on offsets to any value passed in. This stands in contrast to traditional (imperative) rounding where the user must specify exactly what to round a specific input to.
...
|-----------|---------|-------------|
| value | Single, Dataset-aware single | The number to round. |
| range | Dataset, Dataset-aware single | The range of values (for rounding or context). |
| offset | Single | OoM adjustment for single mode. |
| offset_top | Dataset, Dataset-aware single | OoM adjustment for top magnitude(s). |
| offset_other | Dataset, Dataset-aware single | OoM adjustment for other magnitudes. |
| num_top | Dataset, Dataset-aware single | How many top orders of magnitude get `offset_top`. |

### Internal Variables

| Variable | Definition |
|----------|------------|
| current_mag | The magnitude of the input value. |
| max_mag | The largest magnitude in the dataset. |
| oom_offset | Integer part of the offset parameter. |
| fraction | Fractional part of the offset (or 1 if offset is a whole number). |
...
The selected offset is then passed to the rounding logic below.

## Rounding Logic

Given an input `value` and an `offset` parameter:

**Step 1: Find current magnitude**
- current_mag = floor(log10(abs(value)))
- Example: For 87,654,321, `current_mag = floor(log10(87654321)) = floor(7.94) = 7`

**Step 2: Decompose offset into integer and fractional parts**
- oom_offset = trunc(offset)
- fraction = abs(offset - oom_offset), or 1 if zero
- The integer part (`oom_offset`) shifts the magnitude. The fractional part creates "half-step" rounding (e.g., nearest 5M instead of nearest 10M).
- Example: For `offset = -1.5`, `oom_offset = -1` and `fraction = 0.5`

**Step 3: Calculate target magnitude**
- target_mag = current_mag + oom_offset
- Example: `target_mag = 7 + (-1) = 6`

**Step 4: Calculate rounding base**
- base = 10^target_mag × fraction
- Example: `base = 10^6 × 0.5 = 500,000`

**Step 5: Round**
- result = round(value / base) × base
- Example: `round(87654321 / 500000) × 500000 = round(175.3) × 500000 = 175 × 500000 = 87,500,000`